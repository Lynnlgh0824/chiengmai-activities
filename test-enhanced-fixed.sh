    ...
    try:
        ...
        if response.status_code != 200:
            ...
            if '<div id="root">' in html:
                ...
                issues.append("缺少React根容器")
                print_error("缺少React根容器")
            ...
            # 通过API验证数据是否可用
            try:
                api_response = requests.get(f"{API_URL}/activities", timeout=5)
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            try:
                api_response = requests.get(f"{API_URL}/activities", timeout=5)
                if api_response.status_code == 200:
                    print_success("活动数据API可访问（日历数据源正常）")
                else:
                    issues.append("活动数据API不可访问")
                    print_error("活动数据API不可访问")
            except:
                issues.append("无法连接活动数据API")
                print_error("无法连接活动数据API")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('success'):
                    ...
                    print_success(f"{name}: 正常 ({count}条数据)")
                else:
...
def check_data_consistency():
    ...
    try:
        ...
        if valid_count > 0 or len(activities) == 0:
...
def suggest_fixes(issues):
    ...
    for issue in issues:
        if '主页' in issue or 'React根容器' in issue:
            print_info(f"• {issue}")
            print_info("  建议: 检查 index.html 和 src/main.jsx")
        elif '日历' in issue:
            print_info(f"• {issue}")
            ...
            print_info(f"• {issue}")
            print_info("  建议: 检查 server.js，确保后端服务运行")
        else:
            print_info(f"• {issue}")
